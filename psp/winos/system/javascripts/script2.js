...
function ApplySettings(){
	...
	if(icookies.document.getElementById('chkul').value=='true'){
		...
		if(icookies.document.getElementById('chkul').value=='false'){
			...
			document.getElementById('imgUserlogin').src="../images/userlogin.png";
		}
	}
	if(icookies.document.getElementById('chkmenu').value=='false'){
		icookies.document.getElementById('menustyle').value='classicMenu';
	}else{
		icookies.document.getElementById('menustyle').value='normalMenu';
	}
...
function winMessage(MessageTitle,MessageIcon,MessageHTML){
	...
	shutdownFade();
	windowMin('');windowMin(2);windowMin(3);
}

function openInBrowser(Iurl){
	openedLink=Iurl;
	setTitle("1","Internet","../apps/internet/index.htm","../images/icons/menu/apps/internet.png",0);
}
