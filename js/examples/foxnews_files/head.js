...
var domainHost = "http:\/\/www.foxnews.com"; // default
	...
	render: function() {
		...
		function FNAuthService() {
			...
			this.createLoginLogout = function(disableEditProfile) {
				var str = [], auth = accntElm.find("#authentication");
				...
				if(as.isLoggedIn()) {
					...
					if (isLegacy) { 
						...
						auth.each(function(){
							if (!as.isThirdParty()) { $(this).html(profileLink()); } else { $(this).css({ display:"none" }); }
						});

						auth.next().each(function(){
							var logoutLink = $('<a href="#">Log Out</a>');
							logoutLink.click(function(){
								try { authentication_service.logout(authentication_service.domain); } catch(err) { showToConsole("[head.authentication] Logout - An error occured: " + err); }
								return false;
							});
						...
						auth.each(function(){
							if (!as.isThirdParty()) { $(this).html(profileLink($(this).text())); } else { auth.css({ display:"none" }); }
						});
						...
						var displayName = (function(){
							var full = as.getDisplayName(), disp = full, title = "", txt = '<span${title}>${name}</span>';
							if (full.length>13) {
					...
					auth.children().find("a").each(function(){
						...
						if (text.indexOf("register")>-1) {
							item.click(function(){
								try { authentication_service.registrationOverlay(this); } catch(err) { showToConsole("[head.authentication] Register - An error occured: " + err); }
								return false;
							});
						} else if (text.indexOf("login")>-1) {
							item.click(function(){
								try { authentication_service.providerOverlay(authentication_service.domain); } catch(err) { showToConsole("[head.authentication] Login - An error occured: " + err); }
								return false;
							});
						}
			...
			this.callback = function(jsonMsg) {
				...
				if(authObj.authentication.status==="ok"){
					if (as.thirdPartyAPI) {
						as.thirdPartyAPI.disqus.login(authObj,function(){ // disqus
							as.thirdPartyAPI.idms.login(authObj,function(){ // idms
...
function getDomainHost(currHost) {
	...
	try {
		...
		var validSubdomains = ["pvw-qa","origin-qa","staging"]; // qa subdomains get relative paths - this is only for V7
