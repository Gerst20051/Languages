...
if (localStorage.hist != undefined) {
	load();
	for (var i = 0; i < hist.length; ++i) got(hist[i]);
}
...
function echo(msg) {
	$("#out-inner").append("<span class='echo'>" + htmlify(msg.toString()) + "</span><br>");
	scrollToEnd();
}
...
function insert(resultIndex, formula, result) {
	...
	$("#out-inner").append("<div class='left'>r" + resultIndex + " = </div><div class='result'><span class='expr'>" + nice + "</span>" + resultOrNothing + "</div>");
	scrollToEnd();
}

function compute(expr) {
	var hex = expr.match(/in hex$/);
	if (hex) expr = expr.replace(/in hex$/, '');
	var oct = expr.match(/in oct$/);
	if (oct) expr = expr.replace(/in oct$/, '');
	echo(expr);
	...
	if (isFunction(expr)) insert(current, expr);
	else {
...
$('#in input').keydown(function(e) {
	...
	if (code == 13) {
		...
		if (histpos < hist.length - 1) ++histpos;
		it.attr('value', hist[histpos]);
	} else if (code == 9) {
	} else {
		return true;
